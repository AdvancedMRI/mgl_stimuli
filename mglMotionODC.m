% mglMotionODC.m
%
%        $Id: mglMotionODC,v 1.5 2007/06/11 
%      usage: mglMotionODC(agCal,varargin)
%         by: alex beckett
%       date: 03/23/16
%  copyright: (c) 2007 Justin Gardner (GPL see mgl/COPYING)
%    purpose: Displays drifting motion dots for ODC experiments
%
%
%	      Can calibrate r/g and give these up for future use
%	      agCal=[]
%	      [~,agCal]=mglMotionODC(agCal);

function [myscreen,agCal] = mglMotionODC(agCal,varargin)

if ~exist('agCal','var') || isempty(agCal);
%     agCal.dots1=[1 0 0];
%     agCal.dots2=[0 1 0];
    agCal=adjustAnaglyphsDark;
end
...
stimulus.dots.color1=agCal.dots1;
stimulus.dots.color2=agCal.dots2;

% stimulus.dots.color1=[1 0 0];
% stimulus.dots.color2=[0 1 0];
stimulus.dots.color3=stimulus.dots.color1+stimulus.dots.color2;
stimulus.dots.color4=[0 0 0];

stimulus.speedFlag=0;
...
task{2}{1}.seglen = [12];
task{2}{1}.parameter.color = [0];
% task{2}{1}.parameter.speed = [0];
...
%   task{2}{1}.seglen = (stimulus.stimulusPeriod/stimulus.stepsPerCycle)/n_dir_segs*ones(1,n_dir_segs);
  task{2}{2}.seglen = ones(1,stimulus.stimulusPeriod*dutyCycle);
    task{2}{2}.seglen = ones(1,stimulus.stimulusPeriod);
    task{2}{2}.seglen = [stimulus.stimulusPeriod,stimulus.stimulusPeriod/2];
    task{2}{2}.seglen = [stimulus.stimulusPeriod,0];


end

% task{2}{2}.parameter.coherence = 1;
% task{2}{2}.parameter.speed = [1 zeros(1,(1/dutyCycle-1))];
% task{2}{2}.parameter.speed = [1 ones(1,(1/dutyCycle-1))];
task{2}{2}.parameter.color = [1 2 3 4];
task{2}{2}.random = 1;
task{2}{2}.numBlocks = 4;
...
task{2}{3}.seglen = [12];
task{2}{3}.parameter.color = [0];
% task{2}{3}.parameter.speed = [0];
% task{2}{3}.parameter.coherence = 0;
task{2}{3}.numBlocks=1;
...
if task.thistrial.color==1;
    stimulus.dots.color=stimulus.dots.color1;
elseif task.thistrial.color==2;
    stimulus.dots.color=stimulus.dots.color2;
elseif task.thistrial.color==3;
    stimulus.dots.color=stimulus.dots.color3;
elseif task.thistrial.color==4;
...
mglGluDisk(0,0,.75,[0 0 0]);
%mglFixationCross(.5);
if stimulus.dots.mask,mglStencilSelect(0);end
stimulus.speedFlag=stimulus.speedFlag+myscreen.frametime;
if stimulus.speedFlag>=1
    %display('flag');
    stimulus.dots.speed=-stimulus.dots.speed;
    stimulus.dots = feval(sprintf('setDotsSpeed%s',stimulus.dots.type),stimulus.dots,stimulus.dots.speed,myscreen);
    stimulus.speedFlag=0;
end
...
%% include d2r in case not on path
