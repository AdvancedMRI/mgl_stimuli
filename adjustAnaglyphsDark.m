...
%
%      usage: [ rgb ] = adjustAnaglyphsDark(  )
%         by: denis schluppeck, modified by alex beckett
%       date: 2009-12-19, 2016-03-28
%        $Id$:
%     inputs: 
%    outputs: rgb
%
...
%
%    this version uses a dark background instead of a gray
%
...
%
%
function [ rgb ]=adjustAnaglyphsDark( varargin  )
...
myscreen.background = [0 0 0];%'black';
...
% init the stimulus
global stimulus1 stimulus2;
myscreen = initStimulus('stimulus1',myscreen);
myscreen = initStimulus('stimulus2',myscreen);

stimulus1.dots.xcenter=5;
stimulus1.dots.rmax=5;
stimulus2.dots.xcenter=-5;
stimulus2.dots.rmax=5;
stimulus1.dots.color=[0 1 0];
stimulus2.dots.color=[1 0 0];


stimulus1 = initDots(stimulus1,myscreen);
stimulus2 = initDots(stimulus2,myscreen);


% stimulus - color increment per button press
stimulus1.colInc = 0.025;
stimulus2.colInc = 0.025;
...
rgb.dots1 = stimulus1.dots.color;
rgb.dots2 = stimulus2.dots.color;
...
global stimulus1 stimulus2
mglClearScreen;
stimulus1 = updateDots(stimulus1,myscreen);
stimulus2 = updateDots(stimulus2,myscreen);
...
if ~isfield(stimulus.dots,'color1'), stimulus.dots.color1 = [0 1 0];,end
if ~isfield(stimulus.dots,'color2'), stimulus.dots.color2 = [1 0 0];,end
...
stimulus.dots.xmin = stimulus.dots.xcenter-(stimulus.dots.width/2);
stimulus.dots.xmax = stimulus.dots.xcenter+stimulus.dots.width/2;
...
mglGluDisk(stimulus.dots.x,stimulus.dots.y,...
	   stimulus.dots.dotsize/12,stimulus.dots.color);
...
c1 = stimulus1.dots.color;
c2 = stimulus2.dots.color;


switch response
 case 1
  %c1 = c1 + [0 -stimulus.colInc 0];
   c1 = c1 + [0 -stimulus1.colInc 0];
 case 2
  %c1 = c1 + [0 +stimulus.colInc 0];
   c1 = c1 + [0 +stimulus1.colInc 0];
 case 3
  c2 = c2 + [-stimulus2.colInc 0 0];
 case 4
  c2 = c2 + [+stimulus2.colInc 0 0];
end
...
stimulus1.dots.color = c1;
stimulus2.dots.color = c2;
